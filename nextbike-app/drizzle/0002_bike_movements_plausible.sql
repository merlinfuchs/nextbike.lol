-- Add duration_seconds and plausible (speed <= 60 km/h) to bike_movements.
-- Drop and recreate the MV since we cannot ALTER to add columns.
DROP MATERIALIZED VIEW IF EXISTS "nextbike"."bike_movements";
...
segments AS (
  SELECT
    ...
    ordered.end_time,
    EXTRACT(EPOCH FROM (ordered.end_time - ordered.created_at))::double precision AS duration_seconds,
    a.id AS area_id,
